Step 13 — Obstacle avoidance
Step 13 will test one obstacle at a time at speed 600. We will test both obstacle colors in both possible driving directions.
This test uses the repository’s real:
CameraManager
green/red masks
find_obstacle_angle_and_draw_lines()
calculate_servo_angle_from_obstacle()
wall-following calculation
choose_output_angle()
ArduinoComms
I checked the repository’s Challenge 2 control loop and obstacle calculations. The test preserves that competition logic while adding a finite distance, preflight detection, automatic stopping, logs, and diagnostic images.
No changes to main.py or the Arduino firmware are needed.
1. What each obstacle should do
Directions are from the robot’s point of view while facing forward.
Obstacle
Required path
Green
Robot passes on the right side of the obstacle
Red
Robot passes on the left side of the obstacle

The --direction argument means the robot’s lap direction. It does not change which side of a colored obstacle is required.
...
Open step13_obstacle_avoidance.py, copy all its contents, and paste them into nano.
Save and exit:
Press Ctrl+O
Press Enter
Press Ctrl+X
...
The script already contains your final calibration:
Servo centre: 82
Left-direction limits: 75–92
Right-direction limits: 72–90
Maximum Step 13 speed: 600
It also uses the forward motor direction established in the working Step 10–12 tests.
4. Prepare the field
...
The program checks the obstacle with the motor stopped before starting. If it cannot confirm the requested color, it will not move.
...
That is a safe abort. Adjust the obstacle position or lighting and repeat the same command.
...
Send me that run’s final output first if any of those occur.
6. Right direction with red
Reposition the robot at the original starting position. Replace the green obstacle with a red obstacle.
Run:
python -u -m step13_tests.obstacle_avoidance_low_speed \
  --direction right \
  --color red \
...
Reposition the robot for a left-direction lap and use only the green obstacle.
Run:
python -u -m step13_tests.obstacle_avoidance_low_speed \
  --direction left \
  --color green \
...
If the avoidance maneuver is correct but the robot stops beside the obstacle before completely clearing it, do not change any steering values. Repeat only that run with:
--steps 2000
For example:
python -u -m step13_tests.obstacle_avoidance_low_speed \
  --direction right \
...
tar -czf step13_for_review.tar.gz \
  step13_results

ls -lh step13_for_review.tar.gz
Send step13_for_review.tar.gz and these four observations:
Right-green: correct side yes; obstacle contact no; wall contact no; strong oscillation no
Right-red: correct side yes; obstacle contact no; wall contact no; strong oscillation no
Left-green: correct side yes; obstacle contact no; wall contact no; strong oscillation no
Left-red: correct side yes; obstacle contact no; wall contact no; strong oscillation no
